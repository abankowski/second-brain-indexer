...
Alert when no successful poll completes for two polling intervals, `delete_pending` is non-zero, authorization errors repeat, or the queue backlog grows. Resolve MCP/model dimension changes as a coordinated external maintenance operation: stop the indexer, set `polling.enabled = false`, prepare and verify the target vector store, update configuration, then restart and call authenticated `POST /indexer/reset-local-index-state` with body `{}` and a new `Idempotency-Key`. Poll its returned full-run ID, then restore the normal polling value only after that run succeeds. A normal full scan skips locally unchanged content and cannot repopulate an externally rebuilt vector store. The reset clears only local `entity_index_state`; it does not rebuild, delete, or write mcp-memory data. It is unavailable when API bearer authentication is disabled.
